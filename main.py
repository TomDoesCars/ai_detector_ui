...
class GUI:
    ...
    def __init__(self):
        ...
        self.select_folder_button = ctk.CTkButton(self.root, text="Select Folder", font=("RobotoRomanLight", 14), command=self.open_folder_dialog)
        self.select_folder_button.place(x=330.0,y=30.0,width=140.0,height=40.0)

        self.select_folder_entry = ctk.CTkEntry(self.root)
        self.select_folder_entry.insert(0,"No Folder Selected")
        self.select_folder_entry.place(x=30.0,y=37.5,width=275.0,height=25.0)

        self.run_analysis_button = ctk.CTkButton(self.root, text="Run Analysis",font=("RobotoRomanLight", 24), command=self.run_analysis)
        self.run_analysis_button.place(x=30.0,y=100.0,width=440.0,height=70.0)

        self.show_graph_button = ctk.CTkButton(self.root, text="Show Graph", font=("RobotoRomanLight", 24), command=self.graph)
        self.show_graph_button.place(x=30.0,y=200.0,width=205.0,height=70.0)

        self.save_results_button = ctk.CTkButton(self.root, text="Save Results", font=("RobotoRomanLight", 24), command=self.save_results)
        self.save_results_button.place(x=265.0,y=200.0,width=205.0,height=70.0)

        self.status_label = ctk.CTkLabel(self.root, text="Program options:", anchor="w")
        self.status_label.place(x=30.0,y=300.0,width=440.0,height=25.0)

        self.gpt2_option_checkbox = ctk.CTkCheckBox(self.root, text="Use OpenAI GPT-2 Output Detetector", font=("RobotoRomanLight", 14))
        self.gpt2_option_checkbox.place(x=30.0,y=325.0)
        self.gpt2_option_checkbox.select()

        self.ai_cheat_check_option_checkbox = ctk.CTkCheckBox(self.root, text="Use AICheatCheck", font=("RobotoRomanLight", 14))
        self.ai_cheat_check_option_checkbox.place(x=30.0,y=355.0)
        self.ai_cheat_check_option_checkbox.select()

        self.axis_option_checkbox = ctk.CTkCheckBox(self.root, text="Use logarithmic x axis for graph", font=("RobotoRomanLight", 14))
        self.axis_option_checkbox.place(x=30.0,y=385.0)
    
        self.status_label = ctk.CTkLabel(self.root, text="Program status:", anchor="w")
    ...
    def run_analysis(self):
        ...
        if len(files_for_analysis)==0:
            ...
            return None
        if not self.ai_cheat_check_option_checkbox.get() and not self.gpt2_option_checkbox.get():
            self.status_window.delete("0.0","end")
        ...
        def get_score(document: list) -> dict:
            ...
            try:
                if self.gpt2_option_checkbox.get():
    ...
    def graph(self):
        ...
        self.results_fig = px.scatter(self.graphing_df, title="Probability of OpenAI Generation in Document", y="Name", x='Probability', color="Model", log_x=self.axis_option_checkbox.get())
